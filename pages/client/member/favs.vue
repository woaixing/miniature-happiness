					...
					<view class="box pd16_15">
						<view class="flex">
							...
							<view class="order-list-item-r">
								<view style="width: 300upx;" class="ft14 ftw600 cl-main text-over">{{item.data.name}}</view>
								<view class="mt8 ft12 cl-notice">{{item.ulog_time}}</view>
								<view class="flex alcenter cl-orange mt8" >
										<text class="ft16 cl-notice ">第{{item.ulog_nid}}集</text>
								</view>
								<view class="flex alcenter space mt10">
									<view class="btn-small plan" @click="ulog_del(item.ulog_id,item.ulog_type,index)" >删除</view>
									<view class="btn-small" @click="detail(item.ulog_rid,item.ulog_nid)" :style="getBtnStyle">观看</view>
								</view>
	...
	export default{
		...
		methods:{
			...
			getList() {
				...
				uni.request({
					url: this.configs.Url+'api.php/User/favs',
					data: data,
					success: data => {
