...
<script>
	...
	export default {
		...
		onLoad: function(e) {
			...
			this.vdurl=uni.getStorageSync("shareurl")
			this.shareurl=this.configs.Url+'img/bee695baec0b0b01f04e2377a004f752.jpg?v=2'
			if(e.vod_id){
				var qrurl=this.configs.Url+'h5/#/pages/client/tuan/detail?id='+e.vod_id	
				console.log(qrurl);
			}else{
				//var qrurl=this.configs.Url+'/d/?id='+this.userinfo.user_id	
				var qrurl=this.configs.Url+'h5/#/pages/login/reg?uid='+this.userinfo.user_id
				console.log(qrurl);
			}
			
			let img = QR.createQrCodeImg(qrurl, {  
				typeNumber: 10,// typeNumber（值范围是1~40）默认为4，可以设置高一点就不会报上述错误，wxqrcode.js里面有通过这个判断生成内容长度
				errorCorrectLevel: 'H',// errorCorrectLevel（值为：'L','M','Q','H'）默认为M，H为纠错能力最高，依次递减，L最低
			    size: 300//二维码大小  
			})
			this.qrcodeImg = img;
		...
		methods: {
			...
			huizhihaibao() {
				...
				// if(this.shareurl){
				// 	var swiperList={}
				// 	swiperList[0] = this.shareurl
				// 	let goodsData = [];
				// 	swiperList.forEach(item=>{
				// 		if(item.indexOf("http") != -1){
				// 			item=item
				// 		}else{
				// 			item=that.configs.Url+item
				// 		}
				// 		goodsData.push({
				// 			posterImg: item 
				// 		})
			...
			selseimging(hbbj){
				...
				setTimeout(function() { //定时器，海报方法不能立即执行，等页面渲染完成
					...
					context.drawImage(that.vdurl, 0, 0, 280, 380) //电影海报 
					...
					context.fillText('我的邀请码：'+that.userinfo.user_id, 20, 373+yd)
					context.fill()
					...
					context.setFillStyle('#333333')
					context.setFontSize(14)
					context.fillText(that.userinfo.user_nick_name, 80, 405+yd) //昵称
					context.fill()
				
					context.setFillStyle('#666666')
					context.setFontSize(10)
					context.fillText('邀请你加入我们', 80, 425+yd)
