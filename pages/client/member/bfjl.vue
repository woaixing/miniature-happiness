			...
			<view class="order-time-main">
				<view v-for="(item,index) in listData" :key="index" class="pb24">
					<view class="box pd16_15">
						<view class="flex">
							<image mode="aspectFill" class="order-list-item-l" :src="item.pic" style="height: auto; border-radius: 5px;"></image>
							<view class="order-list-item-r">
								<view style="width: 300upx;" class="ft14 ftw600 cl-main text-over">{{item.data.name}}【{{item.ulog_id}}】</view>
								<view class="mt8 ft12 cl-notice">{{item.ulog_time}}</view>
								<view class="flex alcenter cl-orange mt8" >
									<text class="ft12 cl-notice ">【消耗积分：{{item.ulog_points}}】【第{{item.ulog_nid+1}}集】</text>
								</view>
								<view class="flex alcenter space mt10">
									<view class="btn-small plan" @click="ulog_del(item.ulog_id,item.ulog_type,index)" >删除</view>
									<view class="btn-small" @click="detail(item.ulog_rid,item.ulog_nid)" :style="getBtnStyle">重新观看</view>
								</view>
	...
	export default{
		...
		methods:{
			...
			getList() {
				...
				uni.request({
					url: this.configs.Url+'api.php/User/plays',
					data: data,
					success: data => {
