  ...
  <view class="content">
    ...
    <view class="list">
      <view class="list-call">
        ...
        <input class="sl-input" v-model="phone" type="text" placeholder="输入用户名/手机号" />
      </view>
      <view class="list-call">
      ...
      <view class="list-call" v-if="regdata.reg_phone_sms==1">
  ...
  export default {
    ...
    methods: {
      ...
      async getcode() {
		...
		// 请求服务器，发送验证码
		var data ={'to':this.phone,'ac':'phone'}
		let [err,res] =await this.$httpas.post('api.php/User/reg_msg',data);
      ...
      bindLogin() {
        ...
        if (this.code.length != 6 && this.regdata.reg_phone_sms==1){
          uni.showToast({
            icon: 'none',
            title: '验证码不正确'
          });
	  ...
	  async zc(){
	  	let _this=this;
	  	let data = {'user_name':this.phone,'user_pwd':this.password,'user_pwd2':this.password,'to':this.phone,'ac':'phone','code':this.code};
	  	if(this.openid){
	  		data.openid=this.openid
	  		data.nickName=this.nickName
	  		data.avatarUrl=this.avatarUrl
	  	}
	  	let [err,res] =await this.$httpas.post('api.php/User/reg',data);
	  	if (!this.$httpas.errorCheck(err,res)) return;
	  ...
	  getreg() {
	  	let data = {};
	  	uni.request({
	  		url: this.configs.Url+'api.php/User/reg',
	  		data: data,
	  		success: data => {
